...
export default function GitHubContribHeatmap({
    ...
    const [empty, setEmpty] = React.useState(emptyLight);

    React.useEffect(() => {
        (async () => {
            try {
                const r = await fetch(`/api/contributions?user=${encodeURIComponent(user)}`);
    ...
    return (
        <div className="mt-10">
            ...
            <div className="flex flex-col gap-2">
                ...
                <div
                    className="grid"
                    style={{ gridTemplateColumns: `repeat(${weeks.length}, ${box + gap}px)`, gap }}
                    role="img"
